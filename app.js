  ...
  function moveRight() {
    for (let i = 0; i < width * width; i++) {
      if (i % 4 === 0) {
        ...
        // Filter removes all the 0 elements from row
        let filteredRow = row.filter((num) => num);

        // Take away 4 from the length of the row
        let missing = width - filteredRow.length;

        // Make an array that is (4 - the zeros) long and then fill
        // the rest of the row with 0s
        let zeros = Array(missing).fill(0);

        // Make a new row that adds the non-zero elements
        // to the zeros
        // eg. [0, 0].concat(2, 2) = [0, 0, 2, 2]
